...
<script type="text/javascript">
         function cargarClientes(url, tableId, campos) {
    $.ajax({
        url: url,
        method: 'GET',
        dataType: 'json',
        success: function(data) {
            console.log("Datos de " + tableId + ":", data);
            $('#' + tableId + ' tbody').empty();
            
            $.each(data, function(index, cliente) {
                let row = '<tr>';
                // Agrega las celdas basadas en los campos proporcionados para cada tabla
                campos.forEach(function(campo) {
                    row += '<td>' + (cliente[campo] || '') + '</td>';
                });
                row += '</tr>';
...
$(document).ready(function() {
    // Tabla Export (get_data_TEX.php)
    cargarClientes(
        'get_data_TEX.php', 
        'clientes-table_TEX', 
        ['aid_oid', 'cliente', 'vehiculo', 'pedidos_en_proceso', 'pedidos_despachados']
    );

    // Tabla Import (get_data_TIM.php)
    cargarClientes(
        'get_data_TIM.php', 
        'clientes-table_TIM', 
        ['aid_oid', 'cliente', 'contenedores_recibidos', 'contenedores_cerrados']
    );

    // Tabla Picking (get_data_TPK.php)
    cargarClientes(
        'get_data_TPK.php', 
        'clientes-table_TPK', 
        ['aid_oid', 'cliente', 'vehiculo', 'vacio_lleno', 'pedidos_en_proceso', 'pedidos_despachados', 'division_dp', 'fecha_objetivo']
    );

    // Configurar actualización automática de cada tabla cada 5 segundos
