<?php
    ...
    // Verificar que la variable tipo y el id estén presentes en la URL
    if (!isset($_GET['id']) || !isset($_GET['tipo'])) {
        $resultado['error'] = true;
        $resultado['mensaje'] = 'El registro o el tipo no existen';
        die();  // Evita continuar si no hay ID o tipo
    }

    // Validar y asignar el nombre de la tabla basado en el valor de 'tipo'
    $tipo = $_GET['tipo'];
    $tabla = '';
    if ($tipo === 'import') {
        $tabla = 'import';
        ...
        $resultado['error'] = true;
        $resultado['mensaje'] = 'Tipo no válido';
        die(); // Finaliza si el tipo no es válido
    }
    ...
    if (isset($_POST['submit'])) {
        try {
            ...
            $consultaRecordSQL = "INSERT INTO ${tabla}_r 
